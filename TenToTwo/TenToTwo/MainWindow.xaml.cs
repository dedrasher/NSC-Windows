...
namespace NumericSystemConverterApp
{
   ...
   MouseLeftButtonDownEvent,
    new RoutedEventHandler(DeselectListBoxItem));
        ...
        {
            if (AnswerLabel.Text.Length > 0 && e.ChangedButton == MouseButton.Left)
        ...
        {
            Clipboard.Clear();
            Clipboard.SetText(AnswerLabel.Text.Replace(AppLocalize.GetLocalizedString(AppLocalize.TranslateText.Answer, appLanguage), ""));
        }

        private void AnswerLabel_ContextMenuOpening(object sender, ContextMenuEventArgs e)
        {
            if (AnswerLabel.Text.Length > 0)
            {
                var Button = AnswerLabel.ContextMenu.Items[0] as MenuItem;
                Button.Header = AppLocalize.GetLocalizedString(AppLocalize.TranslateText.Copy, appLanguage);
            }
            else
                e.Handled = true;
        }
